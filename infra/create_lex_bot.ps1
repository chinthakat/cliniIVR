...
$RoleArn = "arn:aws:iam::554800146362:role/LexV2ServiceRole_ClinicListener"
...
try {
    $Bot = aws lexv2-models create-bot --bot-name $BotName --role-arn $RoleArn --data-privacy file://data_privacy.json --idle-session-ttl-in-seconds 300 --output json --no-cli-pager | ConvertFrom-Json
    $BotId = $Bot.botId
...
try {
    aws lexv2-models create-bot-locale --bot-id $BotId --bot-version "DRAFT" --locale-id "en_US" --nlu-intent-confidence-threshold 0.40 --no-cli-pager
}
catch {
...
try {
    $Intent = aws lexv2-models create-intent --bot-id $BotId --bot-version "DRAFT" --locale-id "en_US" --intent-name "CatchAll" --sample-utterances "utterance" "I want to" "help" --output json --no-cli-pager | ConvertFrom-Json
    $IntentId = $Intent.intentId
}
catch {
    $IntentId = aws lexv2-models list-intents --bot-id $BotId --bot-version "DRAFT" --locale-id "en_US" --no-cli-pager --query "intentSummaries[?intentName=='CatchAll'].intentId" --output text
...
try {
    $Slot = aws lexv2-models create-slot --bot-id $BotId --bot-version "DRAFT" --locale-id "en_US" --intent-id $IntentId --slot-name "UserQuery" --slot-type-id "AMAZON.FreeFormInput" --value-elicitation-setting file://slot_params.json --output json --no-cli-pager | ConvertFrom-Json
    $SlotId = $Slot.slotId
}
catch {
    $SlotId = aws lexv2-models list-slots --bot-id $BotId --bot-version "DRAFT" --locale-id "en_US" --intent-id $IntentId --no-cli-pager --query "slotSummaries[?slotName=='UserQuery'].slotId" --output text
}
...
aws lexv2-models update-intent --bot-id $BotId --bot-version "DRAFT" --locale-id "en_US" --intent-id $IntentId --intent-name "CatchAll" --sample-utterances "{UserQuery}" --slot-priorities file://priority.json --no-cli-pager

# 5. Build Bot
Write-Host "Building Bot..."
aws lexv2-models build-bot-locale --bot-id $BotId --bot-version "DRAFT" --locale-id "en_US" --no-cli-pager

# Wait for build
Write-Host "Waiting for build..."
do {
    Start-Sleep -Seconds 5
    $Status = aws lexv2-models describe-bot-locale --bot-id $BotId --bot-version "DRAFT" --locale-id "en_US" --query "botLocaleStatus" --output text --no-cli-pager
